...
public class RentStandSucessHandler implements EventHandler<ActionEvent> {
	...
	// constructor
	public RentStandSucessHandler(BorderPane pane, String roomID, TextField cusIDInput, 
	...
	public void handle(ActionEvent event) {
		try {
			...
			CityLodge.numOfRentDayStand(rentDateDT, numOfDays);
			System.out.println(rentDateDT.getNameOfDay());
			System.out.println(rentDateDT.getFormattedDate());
			
			// find the room from rooms list and set it rented and add record
			for(int i = 0; i < CityLodge.getRooms().size(); i++) {
				System.out.println(CityLodge.getRooms().get(i).getRoomID());
			...
			if (e.getReason().compareTo("invalid days for weekend") == 0) {

				AlertBox.display("Error", "Days can only between 3 and 10");
			} else if (e.getReason().compareTo("invalid days for weekday") == 0) {

				AlertBox.display("Error", "Days can only between 2 and 10");
			}
		}
